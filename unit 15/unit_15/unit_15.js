...
let in3 = document.querySelector('.i-3');
const f3 = () => { 
    s3.delete(in3.value);
    console.log(s3);
}
...
const f7 = () => { 
    ...
    let a = in7.split(['']);
    let s7 = new Set(a);
    if ( a.length == s7.size & a.length > 6){
...
const f12 = () => {
    let arrstr12 = str12.split(['']);
    let b = new Set(arrstr12);
    return b;
...
const f13 = () => {
    ...
    let b = new Set(arrstr13);
    let a = {} ;
    // let c = 0;
        for (const iterator of b) {
            let c = 0;
            for (let i = 0; i < arrstr13 .length; i++) {
                if (iterator === arrstr13[i]) {
                    c = c + 1 ;
                 } 
