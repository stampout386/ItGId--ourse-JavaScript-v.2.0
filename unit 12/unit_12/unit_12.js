...
function f10() {
    ...
    for (let i = 0; i < a10.length; i++) {
        for (let k = 0; k < a10[i].length; k++) {
            let d = typeof a10[i][k];
            if (d == 'string') out+= a10[i][k]+' ';
...
function f15() {
    let maxLenght = a15[0].length;
    for (let i = 0; i < a15.length; i++) {
        if (a15[i].length > maxLenght){
            maxLenght = a15[i].length;
        }
        
    }
    document.querySelector('.out-15').innerHTML = maxLenght;
}
